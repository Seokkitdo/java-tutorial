...
public class koreanMenu extends JFrame{
	...
	public koreanMenu() {
		...
		menu_bar.setBounds(0, 0, 1000, 50);
		String[] menuName_bar = {"한식", "양식", "중식", "일식"};
		JButton[] menu_bar_btn = new JButton[menuName_bar.length];
		for(int i=0; i<menuName_bar.length; i++) {
			menu_bar_btn[i] = new JButton(menuName_bar[i]);
			menu_bar_btn[i].setBounds(0+(i*250),0, 250,50);
			menu_bar.add(menu_bar_btn[i]);
		}
